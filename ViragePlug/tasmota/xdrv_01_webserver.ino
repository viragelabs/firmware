/*
  ...
  Copyright (C) 2021 Virage Laboratories Inc.
...
const char HTTP_SCRIPT_ROOT_IDENT[] PROGMEM =
  "var myVar;"
  "var i=1;"

  "function lb(p){"
      "var a='';"
//      "if(lb.arguments.length==1){"
        "a=\"&o=1\";"
//        "clearTimeout(lt);"
//      "}"
      "if(x!=null){x.abort();}"             // Abort if no response within 2 seconds (happens on restart 1)
...
const char HTTP_HEAD_STYLE1[] PROGMEM =
  ...
  "fieldset{background:#%06x;border:0px;}"  // COLOR_FORM, Also update HTTP_TIMER_STYLE
  "p{margin:0.5em 0;}"
...
const char HTTP_FORM_WIFI[] PROGMEM = //wifi config page
  "<fieldset><legend><b></b></legend>"
  "<form method='get' action='wi'>"
  "<p><b>" D_AP1_SSID "</b><input id='s1' placeholder=\"" STA_SSID1 "\" value=\"%s\"></p>"  // Need \" instead of ' to be able to use ' in text (#8489)
  "<p><label><b>" D_AP1_PASSWORD "</b><input type='checkbox' onclick='sp(\"p1\")'></label><input id='p1' type='password' placeholder=\"" D_AP1_PASSWORD "\" value=\"" D_ASTERISK_PWD "\"></p>"
  "<label><b>" D_WEB_ADMIN_PASSWORD "</b><input type='checkbox' onclick='sp(\"wp\")'></label><input id='wp' type='password' placeholder=\"" D_WEB_ADMIN_PASSWORD "\" value=\"" D_ASTERISK_PWD "\"><br><br>"
//  "<p><b>" D_HOSTNAME "</b><input id='h' placeholder='%s' value='%s'></p>"
  "<p><b><input type='hidden' input id='mt' placeholder=\"\" value='%s'></p>"
  "<p><b>" D_HOST "</b><input id='mh' placeholder='%s' value='%s'></p>"
  "<label><input id='b1' type='checkbox'%s><b>" D_MQTT_ENABLE "</b></label><br><br>"
  "<label><input id='b2' type='checkbox'%s><b>" D_MDNS_ENABLE "</b></label><br><br>"
  //"<p><b>" D_CORS_DOMAIN "</b><input id='c' placeholder=\"" CORS_DOMAIN "\" value=\"%s\"></p>";
  "<p><b>" D_DEVICE_NAME "</b><input id='dn' placeholder=\"\" value='%s'></p>";
  //"<p><b>" D_FRIENDLY_NAME1 "</b><br><input id='a1' placeholder=\"\" value='%s'></p>"
  //"<p><b>" D_FRIENDLY_NAME2 "</b><br><input id='a2' placeholder=\"\" value='%s'></p>"
  //"<p><b>" D_FRIENDLY_NAME3 "</b><br><input id='a3' placeholder=\"\" value='%s'></p>";
...
const char HTTP_FORM_RST[] PROGMEM =
  ...
  "<fieldset><legend align='center'><b>&nbsp;" D_RESTORE_CONFIGURATION "&nbsp;</b></legend>";
const char HTTP_FORM_UPG[] PROGMEM =
  ...
  //"</fieldset><br><br>"
  "<fieldset><legend align='center'><b>&nbsp;" D_UPGRADE_BY_FILE_UPLOAD "&nbsp;</b></legend>";
...
const char HTTP_END[] PROGMEM =
  "<div style='text-align:right;font-size:11px;'><hr/><a href='https://www.viragelabs.com' target='_blank' style='color:#aaa;'>" PROJECT " %s " D_BY " Virage Laboratories Inc.</a></div>"
  "</div>"
  "</body>"
  "</html>";

const char HTTP_DEVICE_CONTROL[] PROGMEM = "<td style='width:%d%%'><button onclick='la(\"&o=%d\");'>%s%s</button></td>";  // Control Button Text ?o is related to WebGetArg("o", tmp, sizeof(tmp));
const char HTTP_DEVICE_CONTROL_B[] PROGMEM = "<td style='width:%d%%'><button onclick='lb(\"&o=%d\");'>%s%s</button></td>";
const char HTTP_DEVICE_STATE[] PROGMEM = "<td style='width:%d{c}%s;font-size:%dpx'>%s</div></td>";  // Device State Text {c} = %'><div style='text-align:center;font-weight:

enum ButtonTitle {
  BUTTON_RESTART, BUTTON_RESET_CONFIGURATION,
  BUTTON_MAIN, BUTTON_CONFIGURATION, BUTTON_INFORMATION, BUTTON_FIRMWARE_UPGRADE, BUTTON_CONSOLE, BUTTON_BACKMAIN, BUTTON_BACKCONF,
...
const char kButtonTitle[] PROGMEM =
  ...
  D_MAIN_MENU "|" D_CONFIGURATION "|" D_INFORMATION "|" D_FIRMWARE_UPGRADE "|" D_CONSOLE "|" D_BACKMAIN "|" D_BACKCONF "|"
...
void WebRestart(uint32_t type)
{
  ...
  if (HTTP_MANAGER == Web.state || reset_only) {
    ...
    WSContentSpaceButton(BUTTON_BACKMAIN);
  }
  WSContentStop();
...
void HandleConfiguration(void)
{
  ...
  WSContentSendStyle();
    ...
    WSContentSend_P(PSTR("<tr><td><div style='text-align:center;'><b>" D_CONFIGURATION "</b></div></td></tr><br>"));
  ...
  WSContentSpaceButton(BUTTON_CONSOLE);
...
void HandleTemplateConfiguration(void)
{
  ...
  WSContentSpaceButton(BUTTON_BACKCONF);
  WSContentStop();
}
...
void HandleWifiConfiguration(void)
{
  ...
  WSContentSend_P(HTTP_SCRIPT_ROOT_IDENT, Settings.web_refresh);
  WSContentSendStyle();
  WSContentSend_P(PSTR("<tr><td><div style='text-align:center;'><b>" D_WIFI_PARAMETERS "</b></div></td></tr><br>"));
    ...
    if (Webserver->hasArg("scan")) {
      ...
      if (0 == n) {
        ...
        for (uint32_t i = 0; i < n; i++) {
          ...
          char encryption[20];
          WSContentSend_P(PSTR("<div><a href='#p' onclick='c(this)' style='color:#1fa3ec;'>%s</a>&nbsp;(%d)&nbsp<span class='q'>%s %d%% (%d dBm)</span></div>"),
...
//      WSContentSend_P(PSTR("<div><a href='/wi?scan='>" D_SCAN_FOR_WIFI_NETWORKS "</a></div><br>"));
      WSContentSend_P(PSTR("<a href='/wi?scan='><button type='button' class='button'>" D_SCAN_FOR_WIFI_NETWORKS "</button></a>"));
      WSContentSend_P(PSTR("<div></div>"));  
//<button onclick="window.location.href='https://w3docs.com';">
//    <form action="/wi?scan=">;
//        <input type="submit" value="Scan for Networks" class='button bgrn'/>;
//    </form>;
    }
  ...
  WSContentSend_P(PSTR("<tr>"));
      ...
      bool set_button = ((idx <= MAX_BUTTON_TEXT) && strlen (D_BUTTON_IDENTIFY));
      snprintf_P(stemp2, sizeof(stemp2), PSTR(" %d"), idx);
      WSContentSend_P(HTTP_DEVICE_CONTROL_B, 100 / TasmotaGlobal.devices_present, idx,
        (set_button) ? D_BUTTON_IDENTIFY : (TasmotaGlobal.devices_present < 5) ? D_BUTTON_IDENTIFY : "",
        (set_button) ? "" : (TasmotaGlobal.devices_present > 1) ? stemp2 : "");
      
      }
  WSContentSend_P(PSTR("</tr></table>"));

    // As WIFI_HOSTNAME may contain %s-%04d it cannot be part of HTTP_FORM_WIFI where it will wifi config list
    //wifi config list
    ...
    for (uint32_t i = 0; i < 1 ; i++) {
      ...
      WSContentSend_P(PSTR("<p><b>" D_FRIENDLY_NAME1 "</b><br><input id='a1' placeholder=\"\" value='%s'></p>"),  
        SettingsText(SET_FRIENDLYNAME1 + i));
      }

    for (uint32_t i = 1; i < 2 ; i++) {
      snprintf_P(stemp, sizeof(stemp), PSTR("%d"), i +1);
      WSContentSend_P(PSTR("<p><b>" D_FRIENDLY_NAME2 "</b><br><input id='a2' placeholder=\"\" value='%s'></p>"),
 
        SettingsText(SET_FRIENDLYNAME1 + i));
      }
    for (uint32_t i = 2; i < 3 ; i++) {
      snprintf_P(stemp, sizeof(stemp), PSTR("%d"), i +1);
      WSContentSend_P(PSTR("<p><b>" D_FRIENDLY_NAME3 "</b><br><input id='a3' placeholder=\"\" value='%s'></p>"),
...
void WifiSaveSettings(void)
{
  ...
  SettingsUpdateText(SET_HOSTNAME, (!strlen(tmp)) ? WIFI_HOSTNAME : LowerCase(tmp,(NoAlNumToUnderscore(tmp,tmp))));
  if (strstr(SettingsText(SET_HOSTNAME), "%") != nullptr) {
    SettingsUpdateText(SET_HOSTNAME, WIFI_HOSTNAME);
  }
  WebGetArg("mt", tmp, sizeof(tmp));
  SettingsUpdateText(SET_MQTT_TOPIC, (!strlen(tmp)) ? MQTT_TOPIC : tmp);
  ...
  Settings.flag3.mdns_enabled = Webserver->hasArg("b2");
  WebGetArg("dn", tmp, sizeof(tmp));
  SettingsUpdateText(SET_DEVICENAME, (!strlen(tmp)) ? "" : (!strcmp(tmp,"1")) ? SettingsText(SET_DEVICENAME) : tmp);
  WebGetArg("a1", tmp, sizeof(tmp));
  SettingsUpdateText(SET_FRIENDLYNAME1, (!strlen(tmp)) ? "" : (!strcmp(tmp,"1")) ? SettingsText(SET_FRIENDLYNAME1) : tmp);
  WebGetArg("a2", tmp, sizeof(tmp));
  SettingsUpdateText(SET_FRIENDLYNAME2, (!strlen(tmp)) ? "" : (!strcmp(tmp,"1")) ? SettingsText(SET_FRIENDLYNAME2) : tmp);
  WebGetArg("a3", tmp, sizeof(tmp));
  SettingsUpdateText(SET_FRIENDLYNAME3, (!strlen(tmp)) ? "" : (!strcmp(tmp,"1")) ? SettingsText(SET_FRIENDLYNAME3) : tmp);
  WebGetArg("a1", tmp, sizeof(tmp));
  SettingsUpdateText(SET_BUTTON1, (!strlen(tmp)) ? "" : (!strcmp(tmp,"1")) ? SettingsText(SET_FRIENDLYNAME1) : tmp);
  WebGetArg("a2", tmp, sizeof(tmp));
  SettingsUpdateText(SET_BUTTON2, (!strlen(tmp)) ? "" : (!strcmp(tmp,"1")) ? SettingsText(SET_FRIENDLYNAME2) : tmp);
  WebGetArg("a3", tmp, sizeof(tmp));
  SettingsUpdateText(SET_BUTTON3, (!strlen(tmp)) ? "" : (!strcmp(tmp,"1")) ? SettingsText(SET_FRIENDLYNAME3) : tmp);
...
void HandleInformation(void)
{
  ...
  WSContentSend_P(PSTR("}1" D_LICENSE "}2" D_LICENSE_TEXT));
  WSContentSend_P(PSTR("}1}2&nbsp;"));  // Empty line
  WSContentSend_P(PSTR("}1" D_BLANKLINE "}2" D_LICENSE_TEXT2));
  WSContentSend_P(PSTR("}1" D_BLANKLINE "}2" D_LICENSE_TEXT3));
  WSContentSend_P(PSTR("}1" D_BLANKLINE "}2" D_LICENSE_TEXT4));
  WSContentSend_P(PSTR("}1" D_BLANKLINE "}2" D_LICENSE_TEXT5));
  WSContentSend_P(PSTR("}1" D_BLANKLINE "}2" D_LICENSE_TEXT6));
  WSContentSend_P(PSTR("}1" D_BLANKLINE "}2" D_LICENSE_TEXT7));
  WSContentSend_P(PSTR("}1" D_BLANKLINE "}2" D_LICENSE_TEXT8));
  WSContentSend_P(PSTR("}1" D_BLANKLINE "}2" D_LICENSE_TEXT9));
  WSContentSend_P(PSTR("}1" D_BLANKLINE "}2" D_LICENSE_TEXT10));
  WSContentSend_P(PSTR("}1" D_BLANKLINE "}2" D_LICENSE_TEXT11));
  WSContentSend_P(PSTR("}1}2&nbsp;"));  // Empty line
  ...
  WSContentSend_P(PSTR("}1<hr/>}2<hr/>"));
  WSContentSend_P(PSTR("}1}2&nbsp;"));  // Empty line
  WSContentSend_P(PSTR("}1" D_DEVICE_NAME "}2%s"), SettingsText(SET_DEVICENAME));
