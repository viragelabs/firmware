...
const char HTTP_FORM_WIFI[] PROGMEM = //wifi config
  ...
  "<p><b>" D_DOMOTICZ_IDX "</b><input id='r1' placeholder='0' value='%d'></p>"
...
//    </form>;
    ...
    // As WIFI_HOSTNAME may contain %s-%04d it cannot be part of HTTP_FORM_WIFI where it will exception wifi config wifi list
    WSContentSend_P(HTTP_FORM_WIFI, SettingsText(SET_STASSID1), SettingsText(SET_MQTT_TOPIC), MQTT_HOST, SettingsText(SET_MQTT_HOST), (Settings.flag.mqtt_enabled) ? " checked" : "", (Settings.flag3.mdns_enabled) ? " checked" : "", Settings.domoticz_sensor_idx[5], SettingsText(SET_DEVICENAME));
    ...
    //  WSContentSend_P(PSTR("<p><b>" D_FRIENDLY_NAME " %d</b> (" FRIENDLY_NAME "%s)<br><input id='a%d' placeholder=\"" FRIENDLY_NAME "%s\" value=\"%s\"></p><p></p>"),
    //    i +1,
    //    (i) ? stemp : "",
    //    i,
    //    (i) ? stemp : "",
...
void WifiSaveSettings(void)
{
  ...
  WebGetArg("r1", tmp, sizeof(tmp));
  Settings.domoticz_sensor_idx[5] = (!strlen(tmp)) ? 0 : atoi(tmp);
  WebGetArg("dn", tmp, sizeof(tmp));
  SettingsUpdateText(SET_DEVICENAME, (!strlen(tmp)) ? "" : (!strcmp(tmp,"1")) ? SettingsText(SET_DEVICENAME) : tmp);
  WebGetArg("dn", tmp, sizeof(tmp));
  SettingsUpdateText(SET_FRIENDLYNAME1, (!strlen(tmp)) ? "" : (!strcmp(tmp,"1")) ? SettingsText(SET_DEVICENAME) : tmp);
