...
const char HASS_DISCOVER_SENSOR_HASS_STATUS[] PROGMEM =
  ",\"json_attr_t\":\"%s\","
  "\"val_tpl\":\"{{value_json['" D_JSON_RFRECEIVED "']}}\","
  "\"ic\":\"mdi:radio-tower\"";
...
void NewHAssDiscovery(void)
{
  ...
  snprintf_P(stopic, sizeof(stopic), PSTR("virage/discovery/%s/config"), unique_id);
  ...
  if (!Settings.flag.hass_discovery) {
    snprintf_P(stopic, sizeof(stopic), PSTR("virage/discovery/%s/sensors"), unique_id);
    Response_P(PSTR("{\"sn\":"));
    MqttShowSensor();
  ...
  TuyaDim = TuyaGetDpId((TUYA_MCU_FUNC_DIMMER) + TasmotaGlobal.active_device - 1);
    ...
    if (bitRead(shutter_mask, i-1)) {
      ...
      {                    // SetOption19 - Control Home Assistant automatic discovery (See SetOption59)
          ...
          TryResponseAppend_P(HASS_DISCOVER_RELAY, command_topic, state_topic, value_template, SettingsText(SET_STATE_TXT1), SettingsText(SET_STATE_TXT2));
...
void HAssAnnounceDeviceInfoAndStatusSensor(void)
{
  ...
  {                    // SetOption19 - Control Home Assistantautomatic discovery (See SetOption59)
    ...
    char unique_id[30];
    char *state_topic = stemp1;
    char *state_topicb = stemp2;
    char *state_topicc = stemp3;
    char *availability_topic = stemp4;
    ShowTopic = 0; // Show the new generated topic
    snprintf_P(name, sizeof(name), PSTR("%s status"), SettingsText(SET_DEVICENAME));
    snprintf_P(unique_id, sizeof(unique_id), PSTR("VirageBridge_%06X"), ESP_getChipId());
    GetTopic_P(state_topic, TELE, TasmotaGlobal.mqtt_topic, PSTR(D_RSLT_HASS_STATE));
    GetTopic_P(state_topicb, TELE, unique_id, PSTR(D_RSLT_HASS_STATEB));
    GetTopic_P(state_topicc, TELE, unique_id, PSTR(D_RSLT_HASS_STATE));
    GetTopic_P(availability_topic, TELE, TasmotaGlobal.mqtt_topic, S_LWT);

    Response_P(HASS_DISCOVER_BASE, name, state_topicc);
    TryResponseAppend_P(HASS_DISCOVER_SENSOR_LWT, availability_topic);
    TryResponseAppend_P(HASS_DISCOVER_SENSOR_HASS_STATUS, state_topicb);
    TryResponseAppend_P(HASS_DISCOVER_DEVICE_INFO, unique_id, ESP_getChipId(), SettingsText(SET_DEVICENAME),
                        ModuleName().c_str(), TasmotaGlobal.version, TasmotaGlobal.image_name, TasmotaGlobal.hostname);
    TryResponseAppend_P(PSTR("}"));
  }
  TasmotaGlobal.masterlog_level = ShowTopic;
  MqttPublish(stopic, true);
...
void HAssPublishStatus(void)
{
  ...
  snprintf_P(stemp3, sizeof(stemp3), PSTR("\"%06X\""), received_id);
  ResponseAppend_P(PSTR("{\"" D_JSON_RFRECEIVED "\":%s"), stemp3);
  ResponseAppend_P(PSTR(",\"" D_JSON_VERSION "\":\"%s%s\",\"" D_JSON_BUILDDATETIME "\":\"%s\",\"" D_CMND_MODULE " or " D_CMND_TEMPLATE"\":\"%s\","
