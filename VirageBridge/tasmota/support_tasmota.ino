...
void MqttShowState(void)
{
  ...
  ResponseAppendTime();
  ResponseAppend_P(PSTR(",\"" D_MANUFACTURER "\":\"VirageLabs\""));
  ResponseAppend_P(PSTR(",\"" D_JSON_MODEL "\":\"%s\""), ModuleName().c_str());
  ResponseAppend_P(PSTR(",\"" D_ADMIN_URL "\":\"http://%s.local\""), TasmotaGlobal.hostname);
  ResponseAppend_P(PSTR(",\"" D_IP_ADDRESS "\":\"%s\""), WiFi.localIP().toString().c_str());
...
#ifdef ESP8266
#ifdef USE_ADC_VCC
  dtostrfd((double)ESP.getVcc()/1000, 3, stemp2);
  ResponseAppend_P(PSTR(",\"" D_JSON_VCC "\":%s"), stemp2);
#endif  // USE_ADC_VCC
#endif  // ESP8266
  ...
  if (!TasmotaGlobal.global_state.wifi_down) {
    ...
    ResponseAppend_P(PSTR(",\"" D_JSON_WIFI "\":{\"" D_JSON_AP "\":%d,\"" D_JSON_SSID "\":\"%s\",\"" D_JSON_BSSID "\":\"%s\",\"" D_JSON_CHANNEL "\":%d,\"" D_CMND_HOSTNAME "\":\"%s\",\"" D_CMND_IPADDRESS "\":\"%s\",\"" D_JSON_RSSI "\":%d,\"" D_JSON_SIGNAL "\":%d,\"" D_JSON_LINK_COUNT "\":%d,\"" D_JSON_DOWNTIME "\":\"%s\"}"),
      Settings.sta_active +1, EscapeJSONString(SettingsText(SET_STASSID1 + Settings.sta_active)).c_str(), WiFi.BSSIDstr().c_str(), WiFi.channel(), TasmotaGlobal.hostname, WiFi.localIP().toString().c_str(),
